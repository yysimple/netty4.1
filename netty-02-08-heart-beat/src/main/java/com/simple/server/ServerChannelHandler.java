    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
        ...
        if (evt instanceof IdleStateEvent) {
            ...
            if (e.state() == IdleState.READER_IDLE) {
                System.out.println("服务端提醒：===> 请读取信息...");
                ctx.writeAndFlush("读取等待：客户端你在吗[ctx.close()]{我结尾是一个换行符用于处理半包粘包}... ...\r\n");
                ctx.close();
            } else if (e.state() == IdleState.WRITER_IDLE) {
                System.out.println("服务端提醒：===> 请写入您的数据...");
                ctx.writeAndFlush("写入等待：客户端你在吗{我结尾是一个换行符用于处理半包粘包}... ...\r\n");
            } else if (e.state() == IdleState.ALL_IDLE) {
                System.out.println("服务端提醒：===> 巴拉巴拉...");
                ctx.writeAndFlush("全部时间：我一直都在等待你... ...\r\n");
            }
        }
        // 不是对应的空闲等待事件，那么就将该事件刷新到 pipeline
        ctx.flush();
    }
